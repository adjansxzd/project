...
import { Router } from '@angular/router'; // Yegor
...
export class AddMovieComponent implements OnInit {
  ...
  selectedPreviewFile: File | null = null; // Yegor

  constructor(
    private movieService: MovieService,
    private cdr: ChangeDetectorRef,
    private router: Router // Yegor
  ) {}
  ...
  onPreviewSelected(event: Event): void { // Yegor
    const input = event.target as HTMLInputElement;
    if (input.files && input.files.length > 0) {
  ...
  createMovie(): void { // Yegor
    ...
    if (this.selectedPreviewFile) {
      formData.append('preview', this.selectedPreviewFile); // Yegor
    }

    this.movieService.createMovie(formData).subscribe({
      next: () => {
        alert('Фильм успешно создан');
        this.router.navigate(['/home']); // Yegor
      },
      error: (err) => {
